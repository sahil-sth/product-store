...
const ThemeSelector = () => {
  const [theme, setTheme] = useState(() => {
    //basically means if the page is loaded
    if (typeof window !== "undefined") {
      return localStorage.getItem("theme") || DEFAULT_THEME;
    }
    return DEFAULT_THEME;
  });
  // set the data-theme attribue to theme everytime theme changes
  useEffect(() => {
  ...
  return (
    <div className="dropdown dropdown-end">
      ...
      <ul
        tabIndex={0}
        className="dropdown-content menu bg-base-200 rounde-box z-50 w-56 p-2 shadow-xl max-h-96 overflow-y-auto flex-nowrap"
      >
        {THEMES.map((t) => (
          <li key={t}>
            <button
